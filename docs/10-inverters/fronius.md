...
!!! note "NOTE"
    If you intend to use the [Periodic Reset](../40-setup/10-hardware/periodic_bms_reset.md) option with your battery, make sure to enable the "Defer reset if SOC less than 15%" option to avoid charging from grid if you reached the reserved level, and if Battery Emulator would want to do that at night.
