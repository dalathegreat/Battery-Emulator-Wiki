...
* You can use the [Stark CMR](../30-hardware/stark_cmr.md) hardware which has more CAN channels (Recommended option)
* You can use the [LilyGO T-2CAN](../30-hardware/lilygo_t_2can.md) hardware, which also has two isolated CAN channels (2nd recommended option)
...
1. Log in to the SMA web interface as installer. To perform the battery configuration process you'll need the installer password (or request the PUK on the SMA website).
