...
- If you have a CAN inverter, you need to connect it to a separate 500kbps CAN channel, since the BMS runs at 250kbps on the native CAN
   ...
   - Third option is to use [Stark CMR board](../30-hardware/stark_cmr.md)
   - Fourth option is to use [Double LilyGo](../40-setup/20-software/double_lilygo.md) setup
...
After uploading the code to the Battery-Emulator, you can check cellvoltages, SOC etc. via the [Webserver](../40-setup/20-software/webserver_guide.md)
