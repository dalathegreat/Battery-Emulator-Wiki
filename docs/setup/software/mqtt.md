...
| **Publish heap metric diagnostics** | off | Adds four heap sensors to `/info` and to autodiscovery |
| **Allow remote BMS reset via MQTT** | off | Gates the `BMSRESET` command |
| **Enable Home Assistant auto discovery** | off | Publishes retained HA discovery configs |
| **Home Assistant auto discovery topic** | default `homeassistant` | Base topic for discovery messages |
